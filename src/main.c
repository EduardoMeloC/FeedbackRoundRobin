...
void update(Simulation* simulation){
    ...
    // If current quantum countDown is over, go to next process with remaining burst time (Round Robin)
    ...
    // Current process finished before quantumCountdown is 0
...
int main(int argc, char* argv[]){
    int n_processes = 2;
    Process* p1 = newProcess("P1", 1, 0);
    Process* p2 = newProcess("P2",  1, 2);
